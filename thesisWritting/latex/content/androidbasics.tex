...
Android ships with many subclasses of View, e.g. the \code{TextView} class which displays text content. The view class is also the basis for view groups, to which the layouts, e.g. \code{LinearLayout} and \code{RelativeLayout}, belong to. Views in a window are bundled together as a tree hierarchy with a layout being the top-most root. To add views to an activity or fragment the views can be declared in the corresponding \gls{XML} layout or from code. 
