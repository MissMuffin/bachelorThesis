...
To improve the visiblity of the lines the LinedEditor\todo{ref} class is subclassed from the keyboardless Edittext widget and a background is drawn behind every other line. The LinedEditor is part the RowLinearLayout\todo{ref}, a custom LinearLayout. A LineNumberTextview, a custom Textview, is placed left to the lined editor and displays the line numbers. This is done by setting the same font and text size on both Textview and Edittext.
To suppress the Edittext widget's line wrap behavior its width is set to the value \textit{wrap_content} (see \reflisting{lst:row_editor_xml}). This allows the widget to increase its width when text that exceeds the current width is input.

\begin{listing}[language=XML, caption=Excerpt from row_editor.xml, label=lst:row_editor_xml]
	...
...
The LinearLayout containing the widget has a Scroller\footnote{\url{https://developer.android.com/reference/android/widget/Scroller.html}} attached to it and uses that to scroll its children. The calculation needed for that were adapted from the class TwoDScrollView, written by \cite{clark2010}. The original page
